...
function extractInterviewDetails(text: string) {
  ...
  const notes = [
    isInterview
      ? "Possible interview email detected."
      : "Interview keywords not clearly detected.",
...
router.post("/calendar-event", async (req, res) => {
  try {
    const { title, date, time, durationMinutes, location, meetingLink, notes } =
      req.body;
...
router.post("/", async (req, res) => {
  try {
    const { companyName, roleTitle, location, status, appliedDate, notes } =
      req.body;
